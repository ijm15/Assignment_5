...
final public class AirlineSystem implements AirlineInterface {
  ...
  public boolean deleteRoute(String source, String destination)
    throws CityNotFoundException{
      ...
      // may also need to check that source and destination cities are part of the graph; if not return false

      Route deadRoute1 = null;
      int deadRoute1index = -1;
      for (int i = 0; i < G.v; i++) {
        for (Route e : G.adj(i)) {
        ...
        // no edge between source and destination
        if (deadRoute1 == null) return false;
        G.adj[deadRoute1index].remove(deadRoute1);


        Route deadRoute2 = null;
        int deadRoute2index = -1;
        for (int i = 0; i < G.v; i++) {
        for (Route e : G.adj(i)) {
        ...
        G.adj[deadRoute2index].remove(deadRoute2);
      return true;
    }
  ...
  public void deleteCity(String city) throws CityNotFoundException{
    ...
    for (Route r : retrieveDirectRoutesFrom(city)) {
      deleteRoute(r.source, r.destination);
    }


        String[] hold = new String[cityNames.length];
        for(int i = 0; i < cityNames.length; i++){
          hold[i] = cityNames[i];
        }
